# Contributing to mcp-grade

Thanks for your interest! mcp-grade is intentionally small and opinionated.

## Adding a check

1. Create/edit a module in `mcp_grade/checks/` exposing `CATEGORY` and `run(path) -> list[CheckResult]`.
2. Register it in `mcp_grade/checks/__init__.py` (`ALL_CHECK_MODULES`).
